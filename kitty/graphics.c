...
grman_alloc() {
    ...
    if (!self->images || !self->render_data) {
        PyErr_NoMemory();
        Py_CLEAR(self);
        return NULL;
    }
...
free_load_data(LoadData *ld) {
    ...
    // mmapを解除する
    if (ld->mapped_file) {
        munmap(ld->mapped_file, ld->mapped_file_sz);
    }
    ld->mapped_file = NULL;
    ld->mapped_file_sz = 0;
}
...
img_by_internal_id(GraphicsManager *self, id_type id) {
    // 線形探索してるだけー
    for (size_t i = 0; i < self->image_count; i++) {
        if (self->images[i].internal_id == id) {
 ...
 * @param self GraphicsManagerオブジェクト
 * @param predicate 述語
 * @param skip_image_internal_id 削除対象外の画像オブジェクト内部ID
 */
static inline void
remove_images(GraphicsManager *self, bool (*predicate)(Image *), id_type skip_image_internal_id) {
    ...
    for (size_t i = self->image_count; i-- > 0;) {
        ...
        if (image->internal_id != skip_image_internal_id && predicate(image)) {
            remove_image(self, i);
        }
    }
}
...
/**
 * `remove_images` に指定する述語関数
 *
 * @param image 画像オブジェクト
 */
static bool
trim_predicate(Image *image) {
    return !image->data_loaded || image->refcnt == 0;
}
 ...
 *
 * @param self GraphicsManagerオブジェクト
 * @param storage_limit 記憶領域上限値
 * @param currently_added_image_internal_id 追加された画像内部ID
 */
static inline void
apply_storage_quota(GraphicsManager *self, size_t storage_limit, id_type currently_added_image_internal_id) {
    // 最初にidがある場合でも、未参照の画像を削除します
 ...
 * `add_response` を設定する
 *
 * "{code}: {fmt:args}" な文字列を `add_response` static変数に設定するだけの簡
 * 単なお仕事
 */
static inline void
...
/**
 * アボートマクロ
 *  指定されたコード及び引数を `add_response` staitc変数に設定して `err` ラベル
 *  へジャンプする
 *
 * @param code コード
 * @param arg 可変長引数
 ...
 * @param sz mmapするサイズ ... 0なら`fstat`する
 * @param offset mmapするオフセット
 * @return mmap出来たらtrue
 */
static inline bool
mmap_img_file(GraphicsManager UNUSED *self, Image *img, int fd, size_t sz, off_t offset) {
    if (sz == 0) {
        struct stat s;
        if (fstat(fd, &s) != 0) {
            ABRT(EBADF, "Failed to fstat() the fd: %d file with error: [%d] %s", fd, errno, strerror(errno));
        }
        sz = s.st_size;
    }
    ...
    if (addr == MAP_FAILED) {
        ABRT(EBADF,
             "Failed to map image file fd: %d at offset: %zd with size: %zu with error: [%d] %s",
             fd,
             offset,
             sz,
             errno,
...
inflate_zlib(GraphicsManager UNUSED *self, Image *img, uint8_t *buf, size_t bufsz) {
    ...
    if (!decompressed) {
        fatal("Out of memory allocating decompression buffer");
    }
    z.zalloc = Z_NULL;
    z.zfree = Z_NULL;
    z.opaque = Z_NULL;
    ...
    if ((ret = inflateInit(&z)) != Z_OK) {
        ABRT(ENOMEM, "Failed to initialize inflate with error: %s", zlib_strerror(ret));
    }
    if ((ret = inflate(&z, Z_FINISH)) != Z_STREAM_END) {
        ABRT(EINVAL, "Failed to inflate image data with error: %s", zlib_strerror(ret));
    }
    if (z.avail_out) {
        ABRT(EINVAL, "Image data size post inflation does not match expected size");
    }

    // ロード済データを解放
    free_load_data(&img->load_data);

    // 展開した結果をロード済データとして設定する
...
inflate_png(GraphicsManager UNUSED *self, Image *img, uint8_t *buf, size_t bufsz) {
    png_read_data d = {
        .err_handler = png_error_handler // エラーハンドラー関数の設定
    };
...
#define MAX_DATA_SZ (4u * 100000000u)
    ...
    if (init_img) {
        ...
        case PNG:
            if (g->data_sz > MAX_DATA_SZ) {
                ABRT(EINVAL, "PNG data size too large");
            }
        ...
        case RGBA:
            ...
            if (!img->load_data.data_sz) {
                ABRT(EINVAL, "Zero width/height not allowed");
            }
        ...
        if (transmission == 'd') {
            ...
            img->load_data.buf_capacity = img->load_data.data_sz + (g->compressed ? 1024 : 10);  // 圧縮ヘッダ
        ...
        transmission = g->transmission_type ? g->transmission_type : 'd';
    ...
    switch (transmission) {
    case 'd':      // ダイレクト
        // 画像オブジェクトのロード済データとしてpayloadを格納する
        // サイズが足りなければreallocする
        if (img->load_data.buf_capacity - img->load_data.buf_used < g->payload_sz) {
            if (img->load_data.buf_used + g->payload_sz > MAX_DATA_SZ || fmt != PNG) {
    ...
    case 't':     // 一時ファイル
    case 's':     // POSIX共有メモリ
        if (g->payload_sz > 2048) {
            ABRT(EINVAL, "Filename too long");
        }
        // payloadはファイル名になっているので、そのファイルをmmapする
...
#define IB do { \
    if (img->load_data.buf) { \
        buf = img->load_data.buf; \
        bufsz = img->load_data.buf_used; \
    } else { \
        buf = img->load_data.mapped_file; \
        bufsz = img->load_data.mapped_file_sz; \
    } \
} while (false)
...
#undef IB
        ...
        if (img->load_data.mapped_file) {
            ...
            img->load_data.mapped_file = NULL;
            img->load_data.mapped_file_sz = 0;
        }
    }
    else {
        if (transmission == 'd') {
            ...
            if (img->load_data.mapped_file_sz < img->load_data.data_sz) {
                ...
                img->load_data.data = img->load_data.mapped_file;
            }
        }
    }
    const size_t required_sz = (img->load_data.is_opaque ? 3 : 4) * img->width * img->height;
    if (img->load_data.data_sz != required_sz) {
        ABRT(EINVAL,
             "Image dimensions: %ux%u do not match data size: %zu, expected size: %zu",
             img->width,
             img->height,
             img->load_data.data_sz,
             required_sz);
    }

    // 画像をGPUに転送する
    if (LIKELY(img->data_loaded && send_to_gpu)) {
        send_image_to_gpu(&img->texture_id,
                          img->load_data.data,
                          img->width,
                          img->height,
                          img->load_data.is_opaque,
                          img->load_data.is_4byte_aligned);
        // ロード済データを解放する(だったらImageのメンバに持つ必要あるんか...)
        free_load_data(&img->load_data);
 ...
 *
 * @param self GraphicsManagerオブジェクト
 * @param data_loaded データがロード済かいなか
 * @param iid 内部ID?
 * @return 文字列
 */
static inline const char *
...
update_src_rect(ImageRef *ref, Image *img) {
    // `src_rect` は画像の左上隅を原点とするOpenGL座標[0, 1]
...
handle_put_command(GraphicsManager *self, const GraphicsCommand *g, Cursor *c, bool *is_dirty, Image *img, CellPixelSize cell) {
    ...
    if (!img) {
        set_add_response("ENOENT", "Put command refers to non-existent image with id: %u", g->id);
        return;
    }
    if (!img->data_loaded) {
        set_add_response("ENOENT", "Put command refers to image with id: %u that could not load its data", g->id);
        return;
    }
    ...
    *is_dirty = true;
    ...
    // カーソルを移動し、画面内に収まっていることを確認します TODO 謎
    c->x += ref->effective_num_cols;
    c->y += ref->effective_num_rows - 1;
}

/**
 * `qsort` に指定する比較関数
 *  `z_index` , `image_id` の昇順に並べる。
 *
 * @param a 画像レンダリングデータ
 * @param b 画像レンダリングデータ
 * @return 比較結果
 ...
 * 頂点データを設定する
 *
 * @param rd 画像レンダリングデータ
 * @param ref 画像参照
 * @param dest_rect 画像矩形
 */
static inline void
set_vertex_data(ImageRenderData *rd, const ImageRef *ref, const ImageRect *dest_rect) {

#define R(n, a, b) \
    rd->vertices[n * 4 + 0] = ref->src_rect.a; \
    rd->vertices[n * 4 + 1] = ref->src_rect.b; \
    rd->vertices[n * 4 + 2] = dest_rect->a; \
    rd->vertices[n * 4 + 3] = dest_rect->b;
...
/**
 * 画像をセンタリングするためのGPUデータを構成する
 *
 * @param ans 画像レンダリングデータ
 * @param screen_width_px スクリーン幅のピクセル数
 * @param screen_height_px スクリーン高さのピクセル数
 * @param with 幅 TODO: 多分画像の
 * @param height 高さ TODO: 多分画像の
 */
void
gpu_data_for_centered_image(
    ImageRenderData *ans,
    ...
    // 頂点データ計算する
    static const ImageRef source_rect = {
        .src_rect = {
            .left = 0,
            .top = 0,
            .bottom = 1,
    ...
    const ImageRect r = {
        .left   = -1 + hmargin,
        .right  = -1 + hmargin + width_frac,
        .top    =  1 - vmargin,
        .bottom =  1 - vmargin - height_frac
    };
    set_vertex_data(ans, ref, &r);
}

/**
 * GraphicsManagerのレイヤーを更新する
 *
 * @param self GraphicsManagerオブジェクト
...
grman_update_layers(
    ...
    for (size_t i = 0; i < self->image_count; i++) {
        ...
        for (size_t j = 0; j < image->refcnt; j++) {
            ...
            if (ref->num_rows > 0) {
                ...
                r.bottom = r.top - screen_height * (float)ref->src_height / screen_height_px;
            }
            ...
            if (ref->num_cols > 0) {
                r.right = screen_left + (ref->start_column + (int32_t)ref->num_cols) * dx;
            }
            else {
    ...
    // 表示される画像参照を描画順に並べ替えます（z-index、img）
...
filter_refs(
    ...
    const void *data,
    bool free_images,
    bool (*filter_func)(ImageRef *, Image *, const void *, CellPixelSize),
    CellPixelSize cell
) {
...
scroll_filter_margins_func(ImageRef *ref, Image *img, const void *data, CellPixelSize cell) {
    ...
    if (ref_within_region(ref, d->margin_top, d->margin_bottom)) {
        ref->start_row += d->amt;
        if (ref_outside_region(ref, d->margin_top, d->margin_bottom)) {
            return true;
        }
...
x_filter_func(ImageRef *ref, Image UNUSED *img, const void *data, CellPixelSize cell UNUSED) {
    ...
    return ref->start_column <= (int32_t)g->x_offset - 1 &&
           ((int32_t)g->x_offset - 1) < ((int32_t)(ref->start_column + ref->effective_num_cols));
}
...
/**
 * GraphicsManagerのレイヤーをリサイズする
 *
 * @param self GraphicsManagerオブジェクト
 * @param old_lines 未使用
 * @param lines 未使用
 * @param old_columns 未使用
 * @param columns 未使用
 */
void
grman_resize(GraphicsManager *self,
             index_type UNUSED old_lines,
             index_type UNUSED lines,
             index_type UNUSED old_columns,
             index_type UNUSED columns) {
    // TODO: ん？フラグを立てるだけでいいの？
    self->layers_dirty = true;
}

/**
 * GraphicsManagerのレイヤーを拡大縮小する
 *
 * @param self GraphicsManagerオブジェクト
 * @param cell セルのピクセルサイズ
 */
...
grman_handle_command(
    ...
    case 'q': {
        ...
        if (self->last_init_graphics_command.action == 'T' && image && image->data_loaded) {
            // `put` コマンド処理
            handle_put_command(self, &self->last_init_graphics_command, c, is_dirty, image, cell);
        }
        ...
        if (self->used_storage > STORAGE_LIMIT) {
            apply_storage_quota(self, STORAGE_LIMIT, added_image_id);
        }
        break;
    }
    case 'p':
...
#define R(offset) Py_BuildValue("{sf sf sf sf}", \
                                "left", \
                                r->vertices[offset + 8], \
                                "top", \
                                r->vertices[offset + 1], \
        ...
        PyTuple_SET_ITEM(ans, i,
                         Py_BuildValue("{sN sN sI si sK}", "src_rect", R(0), "dest_rect", R(2), "group_count", r->group_count,
                                       "z_index", r->z_index, "image_id", r->image_id)
                         );
#undef R
    }
    return ans;
...
init_graphics(PyObject *module) {
    if (PyType_Ready(&GraphicsManager_Type) < 0) {
        return false;
    }
    if (PyModule_AddObject(module, "GraphicsManager", (PyObject *)&GraphicsManager_Type) != 0) {
        return false;
    }
