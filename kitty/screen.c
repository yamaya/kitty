...
new(PyTypeObject *type, PyObject *args, PyObject UNUSED *kwds) {
    ...
    if (self != NULL) {
        ...
        self->main_linebuf = alloc_linebuf(lines, columns);
        self->alt_linebuf = alloc_linebuf(lines, columns);
        ...
        if (!init_overlay_line(self, self->columns)) {
            Py_CLEAR(self);
            return NULL;
        }
    }
...
#define setup_cursor() { \
    ...
    LineBuf *n = realloc_lb(self->main_linebuf,
                            lines,
                            columns,
                            &num_content_lines_before,
                            &num_content_lines_after,
                            self->historybuf,
                            &x,
                            &y);
    if (!n) {
...
select_graphic_rendition(Screen *self, unsigned int *params, unsigned int count, Region *region_) {
    if (region_) {
        ...
        if (!region.bottom) {
            region.bottom = self->lines;
        }
        if (!region.right) {
            region.right = self->columns;
        }
...
write_to_child(Screen *self, const char *data, size_t sz) {
    ...
    if (self->test_child != Py_None) {
        write_to_test_child(self, data, sz);
    }
}
...
write_escape_code_to_child(Screen *self, unsigned char which, const char *data) {
    ...
    case DCS:
        ...
        break;
    case CSI:
        prefix = self->modes.eight_bit_controls ? "\x9b" : "\033[";
        suffix = "";
        break;
    case OSC:
...
screen_cursor_back(Screen *self, unsigned int count, int move_direction) {
    ...
    if (move_direction < 0 && count > self->cursor->x) {
        self->cursor->x = 0;
    }
    else {
...
screen_cursor_up(Screen *self, unsigned int count, bool do_carriage_return, int move_direction) {
    ...
    if (move_direction < 0 && count > self->cursor->y) {
        self->cursor->y = 0;
    }
    else {
...
index_selection(Screen *self, Selection *s, bool up) {
    ...
    if (up) {
        ...
        if (s->start_y >= self->lines - 1) {
            s->start_scrolled_by -= 1;
        }
        else {
            s->start_y++;
        }
        if (s->end_y >= self->lines - 1) {
            s->end_scrolled_by -= 1;
        }
        else {
            s->end_y++;
...
screen_reverse_index(Screen *self) {
    ...
    if (self->cursor->y == top) {
        ...
        screen_cursor_up(self, 1, false, -1);
    }
}
...
screen_invert_colors(Screen *self) {
    ...
    if (self->start_visual_bell_at > 0) {
        if (monotonic() - self->start_visual_bell_at <= OPT(visual_bell_duration)) {
            inverted = true;
        }
        else {
            self->start_visual_bell_at = 0;
        }
    }
...
screen_bell(Screen *self) {
    ...
    if (OPT(visual_bell_duration) > 0.0f) {
...
set_dynamic_color(Screen *self, unsigned int code, PyObject *color) {
    if (color == NULL) {
        CALLBACK("set_dynamic_color", "Is", code, "");
    }
    else {
...
set_color_table_color(Screen *self, unsigned int code, PyObject *color) {
    if (color == NULL) {
        CALLBACK("set_color_table_color", "Is", code, "");
    }
    else {
    ...
    case '$':
        ...
        if (strcmp(" q", query) == 0) {
            ...
            case NUM_OF_CURSOR_SHAPES:
                ...
                break;
            case CURSOR_BLOCK:
                shape = self->cursor->blink ? 0 : 2;
                break;
            case CURSOR_UNDERLINE:
                shape = self->cursor->blink ? 3 : 4;
                break;
            case CURSOR_BEAM:
                shape = self->cursor->blink ? 5 : 6;
                break;
            }
        ...
        if (shape > 0) {
            write_escape_code_to_child(self, DCS, buf);
        }
        break;
    } /* switch */
...
static inline void
apply_selection(Screen *self,
                uint8_t *data,
                SelectionBoundary *start,
                SelectionBoundary *end,
                uint8_t set_mask,
                bool rectangle_select){
    if (is_selection_empty(self, start->x, start->y, end->x, end->y)) {
        return;
    }
    if (rectangle_select) {
        ...
        for (index_type y = min_y; y <= max_y; y++) {
            ...
            for (index_type x = x_start; x < xlimit; x++) {
...
screen_apply_selection(Screen *self, void *address, size_t size) {
    ...
    apply_selection(self,
                    address,
                    &self->last_rendered_selection_start,
                    &self->last_rendered_selection_end,
                    1,
                    self->selection.rectangle_select);
...
screen_open_url(Screen *self) {
    ...
    if (url) {
        call_boss(open_url_lines, "(O)", url);
        Py_CLEAR(url);
    }
    else {
        PyErr_Print();
...
deactivate_overlay_line(Screen *self) {
    if (self->overlay_line.is_active && self->overlay_line.xnum && self->overlay_line.ynum < self->lines) {
        ...
        line_reset_cells(line,
                         self->overlay_line.xstart,
                         self->overlay_line.xnum,
                         self->overlay_line.gpu_cells,
                         self->overlay_line.cpu_cells);
...
set_pending_timeout(Screen *self, PyObject *val) {
    if (!PyFloat_Check(val)) {
        PyErr_SetString(PyExc_TypeError, "timeout must be a float");
        return NULL;
    }
...
screen_truncate_point_for_length(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "OI|I", &str, &num_cells, &start_pos)) {
        return NULL;
    }
    if (PyUnicode_READY(str) != 0) {
        return NULL;
...
reset_mode(Screen *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "I|p", &mode, &private)) {
        return NULL;
    }
    if (private) {
        mode <<= 5;
...
_select_graphic_rendition(Screen *self, PyObject *args) {
    ...
    for (int i = 0; i < PyTuple_GET_SIZE(args); i++) {
        params[i] = PyLong_AsUnsignedLong(PyTuple_GET_ITEM(args, i));
    }
...
cursor_up(Screen *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "|Ipi", &count, &do_carriage_return, &move_direction)) {
        return NULL;
    }
...
start_selection(Screen * self, PyObject * args) {
    ...
    if (!PyArg_ParseTuple(args, "II|pi", &x, &y, &rectangle_select, &extend_mode)) {
        return NULL;
    }
...
text_for_selection(Screen *self, PyObject *a UNUSED) {
    ...
    if (start.y == end.y && start.x == end.x) {
        ...
        text_for_range(ans, start, end, self->selection.rectangle_select, true, range_line_, int);
    }
    return ans;
}
...
screen_selection_range_for_line(Screen *self, index_type y, index_type *start, index_type *end) {
    ...
    while (xlimit > 0 && CHAR_IS_BLANK(line->cpu_cells[xlimit - 1].ch)) {
        xlimit--;
    }
    while (xstart < xlimit && CHAR_IS_BLANK(line->cpu_cells[xstart].ch)) {
        xstart++;
    }
    *start = xstart;
...
is_opt_word_char(char_type ch) {
    for (size_t i = 0; i < OPT(select_by_word_characters_count); i++) {
        if (OPT(select_by_word_characters[i]) == ch) {
            return true;
        }
    }
    return false;
...
bool
screen_selection_range_for_word(Screen *self,
                                index_type x,
                                index_type *y1,
                                index_type *y2,
                                index_type *s,
                                index_type *e,
                                bool initial_selection){
    if (*y1 >= self->lines || x >= self->columns) {
        return false;
    }
...
#define is_ok(x) (is_word_char((line->cpu_cells[x].ch)) || is_opt_word_char(line->cpu_cells[x].ch))
    ...
    while (true) {
        while (end < self->columns - 1 && is_ok(end + 1)) {
            end++;
        }
...
screen_history_scroll(Screen *self, int amt, bool upwards) {
    ...
    return false;
} /* screen_history_scroll */

static PyObject *
scroll(Screen *self, PyObject *args) {
    int amt, upwards;

    if (!PyArg_ParseTuple(args, "ip", &amt, &upwards)) {
        return NULL;
    }
    if (screen_history_scroll(self, amt, upwards)) {
        Py_RETURN_TRUE;
    }
    Py_RETURN_FALSE;
}
...
screen_is_selection_dirty(Screen *self) {
    ...
    selection_limits_(selection, &start, &end);
    if (self->last_selection_scrolled_by != self->scrolled_by || start.x != self->last_rendered_selection_start.x ||
        start.y != self->last_rendered_selection_start.y || end.x != self->last_rendered_selection_end.x ||
        end.y != self->last_rendered_selection_end.y || !self->selection_updated_once) {
        return true;
    }
    selection_limits_(url_range, &start, &end);
    if (start.x != self->last_rendered_url_start.x || start.y != self->last_rendered_url_start.y ||
        end.x != self->last_rendered_url_end.x || end.y != self->last_rendered_url_end.y) {
        return true;
    }
    return false;
}
...
screen_update_selection(Screen *self, index_type x, index_type y, bool ended) {
    index_type orig_x = self->selection.end_x, orig_y = self->selection.end_y,
               orig_scrolled_by = self->selection.end_scrolled_by;
    ...
    index_type start, end;
    bool found = false;
    bool extending_leftwards = self->selection.end_y < self->selection.start_y ||
        (self->selection.end_y == self->selection.start_y && self->selection.end_x < self->selection.start_x);
    ...
    case EXTEND_WORD: {
        ...
        if (found) {
            if (extending_leftwards) {
                ...
                self->selection.end_y = y1;
                y1 = self->selection.start_y;
                found = screen_selection_range_for_word(self, self->selection.start_x, &y1, &y2, &start, &end, false);
                if (found) {
                    self->selection.start_x = end;
                    self->selection.start_y = y2;
                }
            }
            else {
                self->selection.end_x = end;
                self->selection.end_y = y2;
                y1 = self->selection.start_y;
                found = screen_selection_range_for_word(self, self->selection.start_x, &y1, &y2, &start, &end, false);
                if (found) {
                    self->selection.start_x = start;
                    self->selection.start_y = y1;
                }
            }
        }
        else {
            self->selection.end_x = orig_x;
            self->selection.end_y = orig_y;
            self->selection.end_scrolled_by = orig_scrolled_by;
        }
        break;
    }
    case EXTEND_LINE: {
        ...
        if (found) {
            if (extending_leftwards) {
                self->selection.start_x = end;
                self->selection.start_y = bottom_line;
            }
            else {
                self->selection.end_x = end;
...
send_escape_code_to_child(Screen *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "is", &code, &text)) {
        return NULL;
    }
...
paste(Screen *self, PyObject *bytes) {
    if (self->modes.mBRACKETED_PASTE) {
        write_escape_code_to_child(self, CSI, BRACKETED_PASTE_START);
    }
    write_to_child(self, PyBytes_AS_STRING(bytes), PyBytes_GET_SIZE(bytes));
    if (self->modes.mBRACKETED_PASTE) {
        write_escape_code_to_child(self, CSI, BRACKETED_PASTE_END);
    }
    Py_RETURN_NONE;
}
