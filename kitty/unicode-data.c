...
is_combining_char(char_type code) {
	...
	switch (code) {
    case 0x300 ... 0x36f: // Block: Combining Diacritical Marks
        return true;
    case 0x483 ... 0x489: // COMBINING CYRILLIC TITLO ~ 
        return true;
    case 0x591 ... 0x5bd:
        return true;
    case 0x5bf:
...
/**
 * 無視する文字(Ignorable Code Points)かどうか
 *
 * > 5.22 Default Ignorable Code Points
 * > 実装は、文字をサポートしていない場合は常に、レンダリングでデフォルトの無視
 * > 可能な文字を無視する必要があります。
 * (see http://www.unicode.org/review/pr-5.html)
 *
 * \param[in] code 文字コード
 * \return 無視するなら真
 */
...
is_ignored_char(char_type code) {
    ...
    case 0x7f ... 0x9f:
        return true;
    case 0xad: // SOFT HYPHEN < Ignorable: 目に見えるグリフはない
        return true;
    case 0x600 ... 0x605: // ARABIC NUMBER SIGN < isIdentifierIgnorable
        return true;
    case 0x61c:
        return true;
    case 0x6dd:
    ...
    case 0x70f:
        return true;
    case 0x8e2: // MONGOLIAN VOWEL SEPARATOR < Space Character
        return true;
    case 0x180e:
        return true;
    case 0x200b ... 0x200c:
...
// L and N categories (127397 codepoints) {{{
switch(code) {
    case 0x30 ... 0x39:
        return true;
    case 0x41 ... 0x5a:
...
is_P_category(char_type code) {
	// P category (punctuation) (792 codepoints) {{{
	switch(code) {
		case 0x21 ... 0x23:
			return true;
		case 0x25 ... 0x2a:
...
/**
 * 結合記号に対応する文字コードポイントを得る
 *
 *  このソフトウェアでは結合記号にインデックスを割当てている（マークと呼称）。
 *  引数 `m` にはそのマークを指定する必要がある。
 *
 *  > 異体字セレクタとそれが付加される文字との組み合わせ、および指定される字形
 *  > は規格で定められており
 *  (via https://ja.wikipedia.org/wiki/%E7%95%B0%E4%BD%93%E5%AD%97%E3%82%BB%E3%83%AC%E3%82%AF%E3%82%BF)
 *
 * \param[in] m 結合記号マーク
 * \return 文字コードポイント
 */
char_type codepoint_for_mark(combining_type m) {
	static char_type map[2275] = {
        0,
...
/**
 * 文字コードポイントから結合記号マークを得る
 *
 *  このソフトウェアでは結合記号にインデックスを割り当てており、それを"マーク"
 *  と呼称している。
 *  このマークとコードポイントは一対一に関係づけしている。
 *
 * \param[in] c 文字コードポイント
 * \return 結合記号のマーク
 */
combining_type mark_for_codepoint(char_type c) {
    // 単純に0ベースのインデックスを算出しているだけ
    // 以下のreturn文のコメントを見よ
	switch (c) {
    case 0:
        return 0;
    case 768: // U+0300: COMBINING GRAVE ACCENT
    case 769:
    case 770:
    case 771:
    case 772:
    ...
    case 879: // (*1)
        return 1 + c - 768;
    case 1155: // U+0483: COMBINING CYRILLIC TITLO
    case 1156:
    case 1157:
    case 1158:
    case 1159:
    case 1160:
    case 1161: // (*2)
        return 113 + c - 1155; // '113' は直前(*1)までのcase節の数
    case 1425:
    case 1426:
    case 1427:
    case 1428:
    ...
    case 1468:
    case 1469:
        return 120 + c - 1425; // '120' は直前(*2)のcase節の数。と言う感じで力技で連番化してるだけ。
