...
set_maximum_wait(monotonic_t val) {
    if (val >= 0 && (val < maximum_wait || maximum_wait < 0)) {
        maximum_wait = val;
    }
}
...
new(PyTypeObject *type, PyObject *args, PyObject UNUSED *kwds) {
    ...
    if (!PyArg_ParseTuple(args, "OO|ii", &death_notify, &dump_callback, &talk_fd, &listen_fd)) {
        return NULL;
    }
    ...
    if (!init_loop_data(&self->io_loop_data)) {
        return PyErr_SetFromErrno(PyExc_OSError);
    }
    if (!install_signal_handlers(&self->io_loop_data)) {
        return PyErr_SetFromErrno(PyExc_OSError);
    }
...
#define needs_write_doc "needs_write(id, data) -> Queue data to be written to child."
    unsigned long id, sz;
    const char *data;
    if (!PyArg_ParseTuple(args, "ks#", &id, &data, &sz)) {
        return NULL;
    }
    if (schedule_write_to_child(id, 1, data, (size_t)sz)) {
        Py_RETURN_TRUE;
    }
    Py_RETURN_FALSE;
}
...
#define shutdown_monitor_doc "shutdown_monitor() -> Shutdown the monitor loop."
    ...
    if (ret != 0) {
        return PyErr_Format(PyExc_OSError, "Failed to join() I/O thread with error: %s", strerror(ret));
    }
    if (talk_thread_started) {
        ret = pthread_join(self->talk_thread, NULL);
        if (ret != 0) {
            return PyErr_Format(PyExc_OSError, "Failed to join() talk thread with error: %s", strerror(ret));
        }
    }
...
do_parse(ChildMonitor *self, Screen *screen, monotonic_t now) {
    ...
    if (screen->read_buf_sz || screen->pending_mode.used) {
        ...
        if (time_since_new_input >= OPT(input_delay)) {
            ...
            if (read_buf_full) {
                wakeup_io_loop(self, false);                 // Ensure the read fd has POLLIN set
            }
...
parse_input(ChildMonitor *self) {
    ...
    if (msg) {
        for (Py_ssize_t i = 0; i < PyTuple_GET_SIZE(msg); i++) {
            PyObject *resp =
                PyObject_CallMethod(global_state.boss, "peer_message_received", "O",
                                    PyTuple_GET_ITEM(PyTuple_GET_ITEM(msg, i), 0));
            int peer_fd = (int)PyLong_AsLong(PyTuple_GET_ITEM(PyTuple_GET_ITEM(msg, i), 1));
            if (resp && PyBytes_Check(resp)) {
                send_response(peer_fd, PyBytes_AS_STRING(resp), PyBytes_GET_SIZE(resp));
            }
            else {
                send_response(peer_fd, NULL, 0);
                if (!resp) {
                    PyErr_Print();
                }
            }
    ...
    for (size_t i = 0; i < count; i++) {
        if (!scratch[i].needs_removal) {
            if (do_parse(self, scratch[i].screen, now)) {
                input_read = true;
            }
        }
...
#define mark_for_close_doc "Mark a child to be removed from the child monitor"
    id_type window_id;
    if (!PyArg_ParseTuple(args, "K", &window_id)) {
        return NULL;
    }
...
#define resize_pty_doc "Resize the pty associated with the specified child"
    ...
    struct winsize dim;
    int fd = -1;
    if (!PyArg_ParseTuple(args, "kHHHH", &window_id, &dim.ws_row, &dim.ws_col, &dim.ws_xpixel, &dim.ws_ypixel)) {
        return NULL;
    }
    children_mutex(lock);
...
#define FIND(queue, count) { \
    ...
    if (fd == -1) {
        FIND(add_queue, add_queue_count);
    }
    if (fd != -1) {
        if (!pty_resize(fd, &dim)) {
            PyErr_SetFromErrno(PyExc_OSError);
        }
    }
    else {
        log_error("Failed to send resize signal to child with id: %lu (children count: %u) (add queue: %zu)",
                  window_id,
                  self->count,
                  add_queue_count);
    }
...
pyset_iutf8(ChildMonitor *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "kp", &window_id, &on)) {
        return NULL;
    }
    children_mutex(lock);
    for (size_t i = 0; i < self->count; i++) {
        if (children[i].id == window_id) {
            found = Py_True;
            if (!set_iutf8(fds[EXTRA_FDS + i].fd, on & 1)) {
                PyErr_SetFromErrno(PyExc_OSError);
            }
            break;
        }
...
collect_cursor_info(CursorRenderInfo *ans, Window *w, monotonic_t now, OSWindow *os_window) {
    ...
    bool cursor_blinking = OPT(cursor_blink_interval) > 0 && os_window->is_focused &&
        (OPT(cursor_stop_blinking_after) == 0 || time_since_start_blink <= OPT(cursor_stop_blinking_after));
...
#ifdef __APPLE__
        if (os_window->is_focused && (OPT(macos_show_window_title_in) & MENUBAR)) {
            cocoa_update_menu_bar_title(w->title);
        }
#endif
        return true;
    }
 ...
 * @param now ？
 * @param active_window_id アクティブなウィンドウのID
 * @param num_visible_windows 可視なウィンドウの数
 * @param all_windows_have_same_bg 全てのウィンドウは同じ背景色であるか否か
 */
static inline bool
...
#define TD os_window->tab_bar_render_data
    ...
    if (TD.screen && os_window->num_tabs >= OPT(tab_bar_min_tabs)) {
        ...
        // セルデータをGPUに転送する
        if (send_cell_data_to_gpu(TD.vao_idx, 0, TD.xstart, TD.ystart, TD.dx, TD.dy, TD.screen, os_window)) {
            needs_render = true;
        }
    }
    if (OPT(mouse_hide_wait) > 0 && !is_mouse_hidden(os_window)) {
        if (now - os_window->last_mouse_activity_at >= OPT(mouse_hide_wait)) {
            hide_mouse(os_window);
        }
        else {
            set_maximum_wait(OPT(mouse_hide_wait) - now + os_window->last_mouse_activity_at);
        }
    }
...
#define WD w->render_data
        ...
        if (w->visible && WD.screen) {
            *num_visible_windows += 1;
            color_type window_bg = colorprofile_to_color(WD.screen->color_profile,
                                                         WD.screen->color_profile->overridden.default_bg,
                                                         WD.screen->color_profile->configured.default_bg);
            if (*num_visible_windows == 1) {
                first_window_bg = window_bg;
            }
            if (first_window_bg != window_bg) {
                all_windows_have_same_bg = false;
            }
            if (w->last_drag_scroll_at > 0) {
                if (now - w->last_drag_scroll_at >= ms_to_monotonic_t(20ll)) {
                    ...
                    set_maximum_wait(now - w->last_drag_scroll_at);
                }
            }
            ...
            if (is_active_window) {
                *active_window_id = w->id;
                ...
                if (w->cursor_visible_at_last_render != WD.screen->cursor_render_info.is_visible ||
                    w->last_cursor_x != WD.screen->cursor_render_info.x ||
                    w->last_cursor_y != WD.screen->cursor_render_info.y ||
            ...
            // セルデータをGPUに転送する
            if (send_cell_data_to_gpu(WD.vao_idx, WD.gvao_idx, WD.xstart, WD.ystart, WD.dx, WD.dy, WD.screen, os_window)) {
                needs_render = true;
            }
...
no_render_frame_received_recently(OSWindow *w, monotonic_t now, monotonic_t max_wait) {
    ...
    if (ans) {
        log_error("No render frame received in %f seconds, re-requesting at: %f",
                  monotonic_t_to_s_double(max_wait),
                  monotonic_t_to_s_double(now));
    }
    return ans;
}
...
cm_thread_write(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "is#", &fd, &buf, &sz)) {
        return NULL;
    }
...
add_python_timer(PyObject *self UNUSED, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "Od|p", &callback, &interval, &repeats)) {
        return NULL;
    }
    unsigned long long timer_id = add_main_loop_timer(s_double_to_monotonic_t(interval),
                                                      repeats ? true : false,
                                                      python_timer_callback,
                                                      callback,
                                                      python_timer_cleanup);
    Py_INCREF(callback);
...
remove_python_timer(PyObject *self UNUSED, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "K", &timer_id)) {
        return NULL;
    }
    ...
    for (size_t i = 0; i < global_state.num_os_windows; i++) {
        ...
        if (w->live_resize.in_progress) {
            ...
            if (w->live_resize.from_os_notification) {
                if (w->live_resize.os_says_resize_complete || (now - w->live_resize.last_resize_event_at) > 1) {
...
process_global_state(void *data) {
    ...
    if (cocoa_pending_actions) {
        ...
        if (cocoa_pending_actions & NEW_OS_WINDOW) {
            call_boss(new_os_window, NULL);
        }
        if (cocoa_pending_actions_wd) {
            if (cocoa_pending_actions & NEW_OS_WINDOW_WITH_WD) {
                call_boss(new_os_window_with_wd, "s", cocoa_pending_actions_wd);
            }
            if (cocoa_pending_actions & NEW_TAB_WITH_WD) {
                call_boss(new_tab_with_wd, "s", cocoa_pending_actions_wd);
            }
            free(cocoa_pending_actions_wd);
...
read_bytes(int fd, Screen *screen) {
    ...
    if (orig_sz >= READ_BUF_SZ) {
        screen_mutex(unlock, read);
        return true;
    }                                                                         // screen read buffer is full
    ...
    while (true) {
        ...
        if (len < 0) {
            ...
            if (errno != EIO) {
                perror("Call to read() from child fd failed");
            }
            return false;
        }
        break;
    ...
    screen_mutex(lock, read);
    if (screen->new_input_at == 0) {
        screen->new_input_at = monotonic();
    }
...
typedef struct {bool kill_signal, child_died;
} SignalSet;
...
io_loop(void *data) {
    ...
    while (LIKELY(!self->shutting_down)) {
        ...
        for (i = 0; i < self->count; i++) {
            ...
            screen_mutex(lock, write);
            fds[EXTRA_FDS +
                i].events = (screen->read_buf_sz < READ_BUF_SZ ? POLLIN : 0) | (screen->write_buf_used ? POLLOUT : 0);
            screen_mutex(unlock, read);
            screen_mutex(unlock, write);
        }
        if (has_pending_wakeups) {
            ...
            if (time_delta >= 0) {
                ret = poll(fds, self->count + EXTRA_FDS, monotonic_t_to_ms(time_delta));
            }
            else {
                ret = 0;
            }
...
#define WAKEUP do { \
    ...
    has_pending_wakeups = false; \
        ...
        if (data_received) {
            ...
            if (has_pending_wakeups && (now = monotonic()) - last_main_loop_wakeup_at > OPT(input_delay)) {
                WAKEUP;
            }
        }
    }
...
accept_peer(int listen_fd, bool shutting_down) {
    ...
    if (fd_idx < PEER_LIMIT && talk_data.reads_capacity < PEER_LIMIT) {
        ...
        talk_data.fds[fd_idx].fd = peer;
        talk_data.fds[fd_idx].events = POLLIN;
        ensure_space_for(&talk_data, reads, PeerReadData, talk_data.num_reads + 1, reads_capacity, 8, false);
        talk_data.reads[talk_data.num_reads] = empty_prd;
        talk_data.reads[talk_data.num_reads++].fd = peer;
...
#define failed(msg) {read_finished = true; log_error("%s", msg); rd->finished = true; rd->close_socket = true; break; \
        ...
        if (rd->fd == s) {
            if (rd->used >= rd->capacity) {
                if (rd->capacity >= 1024 * 1024) {
                    failed("Ignoring too large message from peer");
                }
...
#define failed(msg) {write_finished = true; log_error("%s", msg); wd->finished = true; break; \
        ...
        if (wd->fd == fd) {
            ...
            if (n == 0) {
                failed("send() to peer failed to send any data");
            }
            else if (n < 0) {
                if (errno != EINTR) {
                    perror("write() to peer socket failed with error");
                    failed("");
                }
            }
            else {
...
remove_poll_fd(int fd) {
    ...
    for (size_t i = talk_data.num_listen_fds; i < count; i++) {
        ...
        if (pfd->fd == fd) {
            ...
            if (num_to_right) {
                memmove(talk_data.fds + i, talk_data.fds + i + 1, num_to_right * sizeof(struct pollfd));
            }
            talk_data.num_talk_fds--;
            break;
        }
    ...
    for (ssize_t i = talk_data.num_reads - 1; i >= 0; i--) {
        ...
        if (rd->finished) {
            ...
            if (rd->close_socket) {
                ...
                shutdown(rd->fd, SHUT_RD);
            }
            free(rd->data);
            ssize_t num_to_right = talk_data.num_reads - 1 - i;
            if (num_to_right > 0) {
                memmove(talk_data.reads + i, talk_data.reads + i + 1, num_to_right * sizeof(PeerReadData));
            }
            else {
                talk_data.reads[i] = empty_prd;
            }
            talk_data.num_reads--;
        }
    }
...
prune_finished_writes(void) {
    ...
    for (ssize_t i = talk_data.num_writes - 1; i >= 0; i--) {
        ...
        if (wd->finished) {
            ...
            shutdown(wd->fd, SHUT_WR);
            safe_close(wd->fd);
            free(wd->data);
            ssize_t num_to_right = talk_data.num_writes - 1 - i;
            if (num_to_right > 0) {
                memmove(talk_data.writes + i, talk_data.writes + i + 1, num_to_right * sizeof(PeerWriteData));
            }
            else {
                talk_data.writes[i] = empty_pwd;
            }
...
move_queued_writes(void) {
    while (talk_data.num_queued_writes) {
        ...
        if (fd_idx < PEER_LIMIT && talk_data.num_writes < PEER_LIMIT) {
            ensure_space_for(&talk_data, fds, PollFD, fd_idx + 1, fds_capacity, 8, false);
            talk_data.fds[fd_idx].fd = src->fd;
            talk_data.fds[fd_idx].events = POLLOUT;
...
talk_loop(void *data) {
    ...
    if ((pthread_mutex_init(&talk_data.peer_lock, NULL)) != 0) {
        perror("Failed to create peer mutex");
        return 0;
    }
    if (!init_loop_data(&talk_data.loop_data)) {
        log_error("Failed to create wakeup fd for talk thread with error: %s", strerror(errno));
    }
...
#undef add_listener
    talk_data.fds[talk_data.num_listen_fds].fd = talk_data.loop_data.wakeup_read_fd;
    talk_data.fds[talk_data.num_listen_fds++].events = POLLIN;

    while (LIKELY(!self->shutting_down)) {
        for (size_t i = 0; i < talk_data.num_listen_fds + talk_data.num_talk_fds; i++) {
            talk_data.fds[i].revents = 0;
        }
        ...
        if (ret > 0) {
            ...
            for (size_t i = 0; i < talk_data.num_listen_fds - 1; i++) {
                if (talk_data.fds[i].revents & POLLIN) {
                    if (!accept_peer(talk_data.fds[i].fd, self->shutting_down)) {
                        goto end;
                    }
                }
            }
            if (talk_data.fds[talk_data.num_listen_fds - 1].revents & POLLIN) {
                drain_fd(talk_data.fds[talk_data.num_listen_fds - 1].fd);                                                                // wakeup
            }
            for (size_t i = talk_data.num_listen_fds; i < talk_data.num_talk_fds + talk_data.num_listen_fds; i++) {
                if (talk_data.fds[i].revents & (POLLIN | POLLHUP)) {
                    if (read_from_peer(self, talk_data.fds[i].fd)) {
                        has_finished_reads = true;
                    }
                }
                if (talk_data.fds[i].revents & POLLOUT) {
                    if (write_to_peer(talk_data.fds[i].fd)) {
                        has_finished_writes = true;
                    }
                }
            }
            if (has_finished_reads) {
                prune_finished_reads();
            }
            if (has_finished_writes) {
                prune_finished_writes();
            }
            peer_mutex(lock);
            if (talk_data.num_queued_writes) {
                move_queued_writes();
            }
            peer_mutex(unlock);
        }
            ...
            if (errno != EAGAIN && errno != EINTR) {
                perror("poll() on talk fds failed");
            }
        }
    }
 end:
    ...
    free(talk_data.fds);
    free(talk_data.reads);
    free(talk_data.writes);
    free(talk_data.queued_writes);
    return 0;
} /* talk_loop */
...
add_peer_writer(int fd, const char *msg, size_t msg_sz) {
    ...
    if (talk_data.num_queued_writes < PEER_LIMIT) {
        ensure_space_for(&talk_data,
                         queued_writes,
                         PeerWriteData,
                         talk_data.num_queued_writes + 1,
                         queued_writes_capacity,
                         8,
                         false);
...
send_response(int fd, const char *msg, size_t msg_sz) {
    ...
    if (!add_peer_writer(fd, msg, msg_sz)) {
        shutdown(fd, SHUT_WR);
        safe_close(fd);
    }
    else {
...
safe_pipe(PyObject *self UNUSED, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "|p", &nonblock)) {
        return NULL;
    }
    int fds[2] = {0};
    if (!self_pipe(fds, nonblock)) {
        return PyErr_SetFromErrno(PyExc_OSError);
    }
...
init_child_monitor(PyObject *module) {
    if (PyType_Ready(&ChildMonitor_Type) < 0) {
        return false;
    }
    if (PyModule_AddObject(module, "ChildMonitor", (PyObject *)&ChildMonitor_Type) != 0) {
        return false;
    }
